...
class ImpulseResponseEstimator(object):
    ...
    def __init__(self, duration=5.0, fs=44100):
        ...
        self.fs = int(fs)
        low = 10.0  # Start frequency is always 10 Hz
        high = self.fs / 2  # End frequency is always Nyquist frequency
    ...
    def generate_test_signal(self):
        ...
        # TODO: crop tail at last zero crossing
        impulse *= win
...
def main():
    ...
    arg_parser.add_argument('--speakers', type=str, required=False, default='FL',
                            help='Speaker channel order in test signal sequence as comma separated values. Mono test '
                                 ...
                                 '"FL,FR". Supported channel names are "FL", "FR", "C", "SL", "SR", "BL" and "BR".')
    ...
    file_path = os.path.join(cli_args.dir_path, 'sweep-{t}s-{fs:d}Hz-{bits:d}bit-10.0Hz-{high:.1f}Hz.wav'.format(
        fs=cli_args.fs, t=cli_args.duration, bits=cli_args.bit_depth, high=cli_args.fs/2
    ))
